 ...
 * File modified and submitted by: Ilze
 ************/
...
var app = {
  ...
  receivedEvent: function (id) {
    ...
    function intialiseMap() {
      ...
      map = new H.Map(div, defaultLayers.normal.map)

      // change the zoomin level
      map.setZoom(12)
      // center over Milton Keynes
      var mklat = "52.0342006"
      var mklon = "-0.7748261"
      map.setCenter({ lat: mklat, lng: mklon })

      // map occupies the whole container
    ...
    function updateMatchingStatus(address) {
      ...
      // fetch all the matches for this OUCU
    ...
    function volunteer(oucu, address, start_time, end_time) {
      ...
      var onSuccess = function (data) {
        ...
        if (obj.status == "success") {
          alert(
            "User " +
              oucu +
              " has successfully offered a ride from " +
              address +
              " from " +
              start_time +
              " to " +
      ...
      // Post the oucu, type 0, address, start, end using the "orders" API
    ...
    function request(oucu, address, start_time) {
      ...
      var onSuccess = function (data) {
        ...
        if (obj.status == "success") {
          alert(
            "User " +
              oucu +
              " has successfully requested a ride from " +
              address +
              " from " +
              start_time
          )
      ...
      // Post the oucu, type 1, address, start using the "orders" API
      var uri = "http://137.108.92.9/openstack/taxi/orders"
      var params = {
        OUCU: oucu,
        type: 1,
      ...
      var listSuccess = function (data) {
        ...
        if (obj.status == "success") {
          ...
          $.each(obj.data, function (index, value) {
            ...
            var uri =
              "https://cors-anywhere.herokuapp.com/http://137.108.92.9/openstack/taxi/orders/" +
              value.id +
              "?OUCU=" +
              oucu
    ...
    function TaxiShare() {
      ...
      taxiShareObject.beginUpdatingMap = function () {
        ...
        // every 10 seconds update the map with the current matches. Comment out when testing
      ...
      // shows alternatives
      taxiShareObject.showAlternatives = function () {
        try {
          var url =
            "https://bustimes.org/services/ou-milton-keynes-central-railway-station-open-univ"
      ...
      // takes a photo
      taxiShareObject.photo = function () {
